...
<template>
  <div class="trend-dashboard">
    <div class="card">
      <div class="card-title">{{ totalTitle }}</div>
      <div v-if="loading" class="loading-state">
        <div class="spinner"></div>
      </div>
      <div v-else-if="currentData.length === 0" class="empty-state">{{ emptyText }}</div>
        ...
        <div class="chart-body">
          ...
          <div class="chart-scroll" ref="totalPanelRef" @scroll="() => updateScrollLeft('total')">
            ...
            >
              ...
              <div class="tooltip-value">{{ totalValueLabel }} {{ formatValue(hoverInfo.value) }}</div>
            </div>
          </div>
        </div>
      </div>
        ...
        <div class="chart-body">
          ...
          <div class="chart-scroll" ref="deltaPanelRef" @scroll="() => updateScrollLeft('delta')">
            ...
            <div class="axis-labels" :style="{ width: `${chartWidth}px`, height: '16px' }">
              <span
                v-for="(item, index) in currentData"
                :key="`label-delta-${item.date}-${index}`"
                class="axis-label"
                :style="{ left: `${getX(index, currentData.length)}px` }"
              >
                {{ formatDate(item.date) }}
            ...
            >
              <div class="tooltip-date">{{ hoverInfo.date }}</div>
              <div class="tooltip-value">{{ deltaValueLabel }} {{ formatValue(hoverInfo.value) }}</div>
            </div>
          </div>
        </div>
      </div>
